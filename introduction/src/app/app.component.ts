...
export class AppComponent implements OnInit {
  ...
  onWindowScroll(e) {
        ...
        window.pageYOffset < (0.65*elServices.clientHeight + elAbout.clientHeight)) {
          elToolbar.classList.add('elServices-inverse');
        } else {
          elToolbar.classList.remove('elServices-inverse');
        }

        if (window.pageYOffset >= (0.65*elServices.clientHeight + elAbout.clientHeight) &&
        window.pageYOffset < (elServices.clientHeight + elAbout.clientHeight + elContacts.clientHeight)) {
