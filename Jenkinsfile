        ...
        stage('RELEASE') {
            steps {
                script {
                    ...
                    def ret = sh(script: 'curl -u admin:password123  -s -o /dev/null -w "%{http_code}" http://192.168.99.1:8081/artifactory/api/storage/libs-release-local/org/example/EPAM_FARM/1.7/EPAM_FARM-1.7.pom', returnStdout: true)
                    if (ret == "200") {
        ...
        stage('DEPLOY') {
            steps {
                script {
                    ...
                    def status = sh(script: 'curl -s -o /dev/null -w "%{http_code}" http://192.168.99.1:8087/EPAM_FARM-1.7/', returnStdout: true)
                    if (status != "200") {
                        currentBuild.result = 'FAILURE'
                        error "deploy failed"
                    }
